...
The installer registers the service through NSSM (`nssm.exe`) and controls lifecycle with `sc.exe`.
...
Windows installer default sets `skip_opencode_start = true` to keep SCM service startup stable. Set it to
`false` only when `opencode` is available for the service account and you want Studio to spawn it automatically.
