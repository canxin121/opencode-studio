...
### macOS: 提示 "is damaged and can't be opened" / "已损坏"

这通常表示 macOS Gatekeeper 拦截了从网络下载的（未公证 notarize）应用。

1. 打开 `.dmg`，将 `OpenCode Studio.app` 拖到 `/Applications`。
2. 在 Finder 打开 `/Applications`，右键应用 -> 打开 -> 再确认一次。
3. 如果仍提示已损坏，可移除 quarantine 标记：
