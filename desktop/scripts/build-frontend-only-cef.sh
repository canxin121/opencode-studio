...
if [[ -n "$TARGET" ]]; then
  cargo tauri build --config tauri.conf.frontend.json --target "$TARGET" --features cef "${BUNDLE_ARGS[@]}"
else
  cargo tauri build --config tauri.conf.frontend.json --features cef "${BUNDLE_ARGS[@]}"
fi
