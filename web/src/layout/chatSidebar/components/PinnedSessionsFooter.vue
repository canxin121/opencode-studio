...
const count = computed(() => Math.max(0, Math.floor(props.pinnedSessionsTotal || 0)))
          ...
          @toggle-select="
            (event) =>
              toggleSessionSelected(item.id, {
                event,
                orderedSessionIds: pinnedSessionRows.map((row) => row.id),
              })
          "
