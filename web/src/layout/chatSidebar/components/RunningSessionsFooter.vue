...
const props = defineProps<{
  ...
  page: number
  paging?: boolean
  runningSessionsPageCount: number
  runningSessionsTotal: number
  runningSessionRows: RunningSessionRow[]
...
const count = computed(() => Math.max(0, Math.floor(props.runningSessionsTotal || 0)))
  ...
  <div class="flex-shrink-0 border-t border-sidebar-border/60 bg-sidebar/95">
    <div class="h-10 px-3 flex items-center justify-between gap-2">
      <button
        ...
        :aria-expanded="open"
        aria-label="Toggle running sessions"
        @click="emit('update:open', !open)"
      >
        <component :is="open ? RiArrowDownSLine : RiArrowRightSLine" class="h-4 w-4 text-muted-foreground" />
        <span class="typography-ui-label font-medium text-muted-foreground">Running sessions</span>
      </button>
      <div class="flex items-center gap-2 flex-shrink-0">
        <span class="text-[11px] font-mono text-muted-foreground/70">{{ count }}</span>
        <SidebarPager
          v-if="open && runningSessionsPageCount > 1"
          :page="page"
          :page-count="runningSessionsPageCount"
          :disabled="Boolean(paging)"
          prev-label="Previous running sessions page"
          next-label="Next running sessions page"
    ...
    <div v-if="open" class="px-2 pb-2">
      <div v-if="count === 0" class="px-2 py-2 text-xs text-muted-foreground">No running sessions.</div>
