...
test('files page restores selected file when files view remounts', () => {
  const file = resolve(import.meta.dir, '../src/pages/FilesPage.vue')
  const source = readFileSync(file, 'utf8')

  assert.ok(source.includes("const restoredSelectedFilePath = ref('')"))
  assert.ok(source.includes('uiState.selectedPath'))
  assert.ok(source.includes('async function restoreSelectedFile(rootPath: string, seq: number) {'))
  ...
  assert.ok(source.includes('await restoreSelectedFile(next, seq)'))
})
