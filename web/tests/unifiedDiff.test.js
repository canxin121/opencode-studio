...
test('buildUnifiedMonacoDiffModel builds Monaco-ready before/after text', () => {
  ...
  assert.equal(model.initialTopLine, 2)
  assert.equal(model.originalStartLine, 1)
  assert.equal(model.modifiedStartLine, 1)
  assert.deepEqual(model.originalLineNumbers?.slice(0, 4), [null, 1, 2, 3])
  assert.deepEqual(model.modifiedLineNumbers?.slice(0, 4), [null, 1, 2, 3])
...
test('buildUnifiedMonacoDiffModel keeps multi-file patches in one Monaco view', () => {
  ...
  assert.equal(model.path, 'a.txt')
  assert.equal(model.hasChanges, true)
  assert.equal(model.initialTopLine, 1)
  assert.equal(model.originalStartLine, 1)
  assert.equal(model.modifiedStartLine, 1)
  assert.equal(Array.isArray(model.originalLineNumbers), true)
  assert.equal(Array.isArray(model.modifiedLineNumbers), true)
...
test('buildUnifiedMonacoDiffModel falls back to raw patch when hunks missing', () => {
  ...
  assert.equal(model.initialTopLine, null)
  assert.equal(model.originalStartLine, null)
  assert.equal(model.modifiedStartLine, null)
  assert.equal(model.originalLineNumbers, null)
  assert.equal(model.modifiedLineNumbers, null)
...
test('buildVirtualMonacoDiffModel can compact large snapshots into diff-focused windows', () => {
  ...
  assert.equal(model.hasChanges, true)
  assert.equal(model.initialTopLine, 3)
  assert.equal(model.originalStartLine, null)
  assert.equal(model.modifiedStartLine, null)
